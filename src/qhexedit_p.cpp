...
{
    /* Hex input */
        //int key = int(event->text()[0].toAscii()); // QT4.x QKeyEvent  text() return unicode QString
        int key = event->key(); // QT >4.x,key() does not distinguish capitalized or not capitalized key
